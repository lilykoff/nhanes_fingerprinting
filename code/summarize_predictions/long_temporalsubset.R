...
fnames = read_rds(here::here("data", "lily", "data", "fingerprint_folds_long_temporal.rds"))
# fname_ids = as.numeric(sub(".rds.*", "", files))
#
# fnames %>%
#   filter(!(id %in% fname_ids))
#
# filenames %>%
#   filter(!(id %in% fname_ids))
...
outfiles = paste("prediction_res_temporal_", dirnums, "longsubset.rds", sep = "")
pred_dirs = paste(
