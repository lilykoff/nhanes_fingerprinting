...
get_density = function(subject, df = df){
  ...
  x = try({
    walking_df =
      readr::read_csv(here::here("data", "lily", "data", "fingerprint_data_10", idf$version, paste0(idf$id, ".csv.gz")))
...
if(!file.exists(here::here(outname)) | force){
  ...
  outname = paste0("grid_data_fold_", ifold, ".csv.gz")
  readr::write_csv(grid_data_df, here::here("data", "lily", "data", "grid_cell_data_fine", outname))
