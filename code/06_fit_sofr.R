...
if(size < 500){
  ...
  outfile = here::here("data", "lily", "data", "fingerprint_res", paste0(size, "fnl"), paste0("fold_", ifold, ".rds"))
  dir = dirname(outfile)
  ...
  for(f in folds$fold){
    ...
    for(id in ids){
      ...
      i = i + 1
      outfile = here::here("data", "lily", "data", "fingerprint_res", paste0(size, "fnl"), paste0(id, ".rds"))
      dir = dirname(outfile)
      if(!dir.exists(dir)){
        dir.create(dir, recursive = TRUE)
      }
