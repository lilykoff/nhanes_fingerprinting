...
png(here::here("manuscript", "figs_final", "acc100_500.png"), width = 8, height = 6, units = "in",
    res = 500)
# print(p)
...
all_logistic %>%
  ...
  group_by(n, paradigm) %>%
  summarize(across(starts_with("rank"), median), .groups = "drop") %>%
  pivot_longer(cols = contains("rank"), names_to = "metric", values_to = "value") %>%
  mutate(metric = case_when(
    metric == "rank5pct" ~ "Rank 5%",
    metric == "rank1pct" ~ "Rank 1%",
    metric == "rank5" ~ "Rank 5",
    .default = "Rank 1"
  )) %>%
  ggplot(aes(x = n, y = value, color = metric, group = metric, label = metric)) +
  geom_jitter(width = 25, size = 2.5) +
  geom_textline(linewidth = 1.1, size = 4, text_smoothing = 5, vjust=-.4, show.legend = FALSE) +
  ...
  scale_color_manual(values = c("#FF7F00FF", "#FFBF7FFF", "#654CFFFF", "#CCBFFFFF"),
                     labels = c("Rank 1", "Rank 1%", "Rank 5", "Rank 5%")) +
...
all_logistic %>%
  ...
  ggplot(aes(x = n, y = value, color = metric, group = metric, label = metric)) +
  geom_jitter(width = 25, size = 2.5) +
  geom_labelpath(linewidth = 1.1, size = 4, vjust = -.5, text_smoothing = 20, show.legend = FALSE, label.padding = unit(0.2, "lines")) +
  facet_grid(. ~ paradigm, scales = "free_x") +
  scale_x_continuous(breaks = c(100, 500, 1000, 2500, 5000, 10000, 10770, 13367),
                     labels = c(1, 5, 10, 25, 50, 100, 108, 133)) +
  scale_y_continuous(breaks = seq(0, 100, 10)) +
...
png(here::here("manuscript", "figs", "acc_pct.png"), width = 9, height = 6, res = 500, units = "in")
p
dev.off()
