...
get_density = function(subject, df = df){
  ...
  x = try({
    ...
    day_test =
      day_df %>%
      filter(n >= (30*60*.25) & day > day_train) %>%
      slice_sample(n = 1)  %>%
      pull(day)
    ...
    test_start = sample(x = 1:(nrow(test)-((30*60*.25) - 1)), size = 1)

    test = test %>% slice(test_start:(test_start + ((30*60*.25) - 1)))
    ...
    train_start = sample(x = 1:(nrow(train)-((30*60*.75) - 1)), size = 1)

    train = train %>% slice(train_start:(train_start + ((30*60*.75) - 1)))
