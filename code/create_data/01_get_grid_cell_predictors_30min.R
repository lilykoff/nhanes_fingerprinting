...
get_density = function(subject, df = df){
  ...
  x = try({
    ...
    if(nrow(df_small) >= 1800) {
      set.seed(123)
      density =
        df_small %>%
        sample_n(size = 1800, replace = FALSE) %>%
        mutate(id = subject) %>%
...
if(!file.exists(here::here("data", "lily", "data", "grid_cell_data_30", outname)) | force){
  grid_data_list =
  ...
  outname = paste0("grid_data_fold_", ifold, ".csv.gz")
  readr::write_csv(grid_data_df, here::here("data", "lily", "data", "grid_cell_data_30", outname))
