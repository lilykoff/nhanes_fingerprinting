...
if(!dir.exists(here::here("data", "lily", "data", "grid_cell_data_long_temporal"))){
  dir.create(here::here("data", "lily", "data", "grid_cell_data_long_temporal"))
}
...
get_density = function(subject, df = df){
  ...
  x = try({
    ...
    test_start = sample(x = 1:(nrow(test)-89), size = 1)

    test = test %>% slice(test_start:(test_start + 89))
    ...
    train_start = sample(x = 1:(nrow(train)-269), size = 1)

    train = train %>% slice(train_start:(train_start + 269))
...
if(!file.exists(here::here("data", "lily", "data", "grid_cell_data_long_temporal", outname)) | force){
  grid_data_list =
  ...
  readr::write_csv(grid_data_df, here::here("data", "lily", "data", "grid_cell_data_long_temporal", outname))
