...
get_density = function(subject, df = df){
  ...
  x = try({
    ...
    # if there are at least 6 mins of data, sample 6 mins randomly (so that everyone has same amount of walking)
    if(nrow(df_small) >= 360) {
      set.seed(123)
      density =
        df_small %>%
        sample_n(size = 360, replace = FALSE) %>%
        mutate(id = subject) %>%
...
if(!file.exists(here::here("data", "lily", "data", "grid_cell_data_big", outname)) | force){
  grid_data_list =
  ...
  outname = paste0("grid_data_fold_", ifold, ".csv.gz")
  readr::write_csv(grid_data_df, here::here("data", "lily", "data", "grid_cell_data_big", outname))
