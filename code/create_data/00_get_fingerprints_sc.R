...
for (index in seq(max_n)) {
  ...
  files = c(idf$csv_file, idf$stepcount_file)
  ...
  if (!all(file.exists(outfiles)) && all(file.exists(files)) | force) {
    x = try({
      ...
      sc_file = readr::read_csv(idf$stepcount_file)
      # filter to seconds where steps
      sc_file_walking = sc_file %>% filter(steps > 0 & walking)
      # read in csv file with raw data
      min_file = read_80hz(idf$csv_file, progress = FALSE)

      sc_file_walking = sc_file_walking %>%
        mutate(end = time + as.period(9, "seconds"))

      sc_walking = sc_file_walking %>%
        rowwise() %>%
        transmute(steps, sec = list(seq(time, end, by = "sec"))) %>%
        unnest(sec) %>%
        ungroup()
      # filter raw data to just seconds with walking
      walking_df = min_file %>%
        mutate(second = lubridate::floor_date(HEADER_TIMESTAMP, unit = "seconds")) %>%
        filter(second %in% sc_walking$sec)
