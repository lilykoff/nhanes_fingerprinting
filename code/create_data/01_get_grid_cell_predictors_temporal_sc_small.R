...
ref_file = read_rds(here::here("data/lily/data/fingerprint_folds_temporal2.rds"))
ref_ids = ref_file$id
...
out_file = here::here("data", "lily", "data", "grid_cell_data_sc_temporal_small", paste0("grid_data_fold_", ifold, ".csv.gz"))
if (!dir.exists(dirname(out_file))) {
  dir.create(dirname(out_file), recursive = TRUE)
}
if(!file.exists(out_file) || force){
  ids_temp = df$id[df$id %in% ref_ids]
  grid_data_list =
    map(.x = ids_temp, .f = get_density, df = df)
