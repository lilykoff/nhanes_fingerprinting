...
for(f in folds$fold){
  ...
  if(length(ids) > 0) {
    ...
    outfiles =
      here::here("data", "lily", "data", "fingerprint_res_scs", size, paste0(ids, ".rds"))
    ...
    if(size == 13367){
      ...
      dat_nzv_test = read_rds(here::here("data", "lily", "data", "dat_nzv_test_scs.rds")) %>%
        mutate(id = as.character(id)) %>%
        filter(id %in% ids_all)
    } else {
    ...
    for(id in ids){
      ...
      i = i + 1
      outfile = here::here("data", "lily", "data", "fingerprint_res_scs", size, paste0(id, ".rds"))
      dir = dirname(outfile)
      if(!dir.exists(dir)){
