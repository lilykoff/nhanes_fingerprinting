...
for(f in folds$fold){
  ...
  if(length(ids) > 0) {
    ...
    ids_all =
      ...
      dat_nzv = read_rds(here::here("data", "lily", "data", "dat_nzv_train_random.rds")) %>%
        mutate(id = as.character(id)) %>%
        filter(id %in% ids_all)
      dat_nzv_test = read_rds(here::here("data", "lily", "data", "dat_nzv_test_random.rds")) %>%
        mutate(id = as.character(id)) %>%
        filter(id %in% ids_all)
    ...
    for(id in ids){
      ...
      i = i + 1
      outfile = here::here("data", "lily", "data", "fingerprint_res", paste0(size, "randomwtd"), paste0(id, ".rds"))
      dir = dirname(outfile)
      if(!dir.exists(dir)){
