...
size = get_input()
# size = 100
# need to do NZV after reading in files
...
outfile_temporal = here::here("data", "lily", "data", "fingerprint_res_scs", size, paste0("fold_", ifold, ".rds"))
...
if(!file.exists(outfile_temporal) || force){
  ...
  folds_to_read = unique(fdf$fold_orig)

  folds_to_read = 1:200

  pred_df = map(.x = folds_to_read, .f = function(fold){
    res = try({read_csv(here::here("data", "lily", "data", "grid_cell_data_sc_small", paste0("grid_data_fold_", fold, ".csv.gz"))) %>%
        filter(id %in% fdf$id)})
    res
  })
  ...
  set.seed(123)
  initialsplit = initial_split(pred_df, prop = 3/4, strata = id)
