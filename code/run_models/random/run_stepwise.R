...
fit_model = function(subject, train, test) {
  ...
  both_model = step(mod, direction = "both")
  pred = predict.glm(both_model, newdata = tmp_test, type = "response")
  return(pred)
}
...
fit_model = function(subject, train, test) {
  ...
  # Start with fewer variables if possible
  mod <- glm(class ~ ., data = tmp, family = binomial(link = "logit"))

  # Suppress output and set lower k to be less aggressive
  both_model <- step(mod, direction = "both", trace = 0, k = log(nrow(tmp)))

  pred <- predict(both_model, newdata = tmp_test, type = "response")
  return(pred)
}
...
for(f in folds$fold){
  ...
  if(length(ids) > 0) {
    ...
    for(id in ids){
      ...
      i = i + 1
      outfile = here::here("data", "lily", "data", "fingerprint_res_stepwise", size, paste0("fold_", ifold, ".rds"))
      dir = dirname(outfile)
      if(!dir.exists(dir)){
