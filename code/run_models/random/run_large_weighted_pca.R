...
for(f in folds$fold){
  ...
  if(length(ids) > 0) {
    ...
    ids_all =
      ...
      dat_pca = read_rds(here::here("data", "lily", "data", "dat_train_pca.rds")) %>%
        mutate(id = as.character(id)) %>%
        filter(id %in% ids_all)
      dat_test_pca = read_rds(here::here("data", "lily", "data", "dat_test_pca.rds")) %>%
        mutate(id = as.character(id)) %>%
        filter(id %in% ids_all)
    ...
    for(id in ids){
      ...
      i = i + 1
      outfile = here::here("data", "lily", "data", "fingerprint_res", paste0(size, "wtd_pca"), paste0(id, ".rds"))
      dir = dirname(outfile)
      if(!dir.exists(dir)){
      ...
      if(!file.exists(outfile) | force){
        x = try({
          ...
          preds = fit_model(subject = id, train = dat_pca, test = dat_test_pca) %>% janitor::clean_names()
