...
outfile_temporal = here::here("data", "lily", "data", "fingerprint_res_sc", size, paste0("fold_", ifold, ".rds"))
...
if(!file.exists(outfile_temporal) || force){
  ...
  pred_df = map_dfr(.x = folds_to_read, .f = function(fold){
    x = read_csv(here::here("data", "lily", "data", "grid_cell_data_sc", paste0("grid_data_fold_", fold, ".csv.gz"))) %>%
      filter(id %in% fdf$id)
    x
  })
