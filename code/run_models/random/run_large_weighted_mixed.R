...
ifold = get_fold()
size = get_input()
filenames = read_rds(here::here("data", "lily", "data", "fingerprint_folds_mixed.rds"))
...
for(f in folds$fold){
  ...
  if(length(ids) > 0) {
    ...
    dat_nzv = read_rds(here::here("data", "lily", "data", "dat_nzv_train_mixed.rds")) %>%
      mutate(id = as.character(id)) %>%
      filter(id %in% ids_all)
    dat_nzv_test = read_rds(here::here("data", "lily", "data", "dat_nzv_test_mixed.rds")) %>%
      mutate(id = as.character(id)) %>%
      filter(id %in% ids_all)
    ...
    for(id in ids){
      ...
      i = i + 1
      outfile = here::here("data", "lily", "data", "fingerprint_res", paste0(size, "mixedwtd"), paste0(id, ".rds"))
      dir = dirname(outfile)
      if(!dir.exists(dir)){
