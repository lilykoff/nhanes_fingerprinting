...
# f = folds$fold[1]
outfile_temporal = here::here("data", "lily", "data", "fingerprint_res_temporal_30min", size, paste0("fold_", ifold, ".rds"))
...
if(!file.exists(outfile_temporal) || force){
  ...
  pred_df = read_csv(here::here("data/lily/data/all_grid_cells_temporal_30min.csv.gz"))
  pred_df = pred_df %>% filter(id %in% fdf$id)


  data_train = pred_df %>% filter(data == "train")
  data_test = pred_df %>% filter(data == "test")
