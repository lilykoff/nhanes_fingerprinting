...
filenames =
  filenames %>%
  mutate(fold = rep(1:200, each = 54)[1:10770])

files = list.files(here::here("data", "lily", "data", "preds_10770"),
                   recursive = TRUE,
                   full.names = TRUE)
...
if(!file.exists(here::here("data", "lily", "data", "preds_10770.rds"))){
  all_preds =
    map_dfr(files, read_rds)
} else{
  all_preds = read_rds(here::here("data", "lily", "data", "preds_10770.rds"))
}
...
# write_rds(all_preds, here::here("data", "lily", "data", "preds_10770.rds"))

if(!dir.exists(here::here("data", "lily", "data", "10770_boosted"))){
  dir.create(here::here("data", "lily", "data", "10770_boosted"),
             recursive = TRUE)
}

data_train = read_rds(here::here("data", "lily", "data", "dat_nzv_train_temporal2.rds")) %>%
  mutate(id = as.character(id))
data_test = read_rds(here::here("data", "lily", "data", "dat_nzv_test_temporal2.rds")) %>%
  mutate(id = as.character(id))
...
outfile = here::here("data", "lily", "data", "10770_boosted", paste0("fold_", ifold, ".rds"))
...
# test = read_rds(here::here("data", "lily", "data", "preds_10770", paste0(66080, ".rds")))
