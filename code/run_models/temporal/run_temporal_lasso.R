...
fit_model = function(subject, train, test) {
  ...
  final_rf = finalize_workflow(lr_wf, best_acc)
  ...
  preds = final_res %>%
    collect_predictions() %>%
    pull(.pred_1)
  rm(final_res); rm(lr_res); rm(final_rf)
  return(preds)
}

outfile_temporal = here::here("data", "lily", "data", "fingerprint_res_temporal2", paste0(size, "lasso"), paste0("fold_", ifold, ".rds"))
