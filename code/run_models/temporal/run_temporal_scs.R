...
outfile_temporal = here::here("data", "lily", "data", "fingerprint_res_temporal_scs", size, paste0("fold_", ifold, ".rds"))
...
if(!file.exists(outfile_temporal) || force){
  ...
  pred_df = map(.x = folds_to_read, .f = function(fold){
    res = try({read_csv(here::here("data", "lily", "data", "grid_cell_data_sc_temporal_small", paste0("grid_data_fold_", fold, ".csv.gz"))) %>%
      filter(id %in% fdf$id)})
    res
  })
