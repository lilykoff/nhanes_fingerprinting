...
filenames = readRDS(here::here("data", "lily", "data", "fingerprint_folds_temporal_30min.rds"))
...
# out_files = list.files(here::here("data", "lily", "data", "fingerprint_res_temporal_30min",
#                                   size))
#
# length(out_files)
# size
#
# ids = sub(".rds.*", "", out_files); ids
#
# filenames %>% filter(!(id %in% as.numeric(ids)))
# f = folds$fold[1]
i = 1
if (!is.na(ifold)) {
  ids = filenames %>%
...
if(length(ids) > 0){
  ...
  dat_nzv = read_rds(here::here("data", "lily", "data", "dat_nzv_train_temporal_30min.rds")) %>%
    mutate(id = as.character(id)) %>%
    filter(id %in% ids_all)
  dat_nzv_test = read_rds(here::here("data", "lily", "data", "dat_nzv_test_temporal_30min.rds")) %>%
    mutate(id = as.character(id)) %>%
    filter(id %in% ids_all)
  ...
  for(id in ids){
    ...
    i = i + 1
    outfile = here::here("data", "lily", "data", "fingerprint_res_temporal_30min", paste0(size, "wtd"), paste0(id, ".rds"))
    dir = dirname(outfile)
    if(!dir.exists(dir)){
