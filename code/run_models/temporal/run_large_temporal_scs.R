...
for(f in folds$fold){
  ...
  if(length(ids) > 0) {
    ...
    outfiles =
      here::here("data", "lily", "data", "fingerprint_res_temporal_scs", size, paste0(ids, ".rds"))
    ...
    if(!all(file.exists(outfiles)) || force) {
      ...
      for(id in ids){
        ...
        i = i + 1
        outfile = here::here("data", "lily", "data", "fingerprint_res_temporal_scs", size, paste0(id, ".rds"))
        dir = dirname(outfile)
        if(!dir.exists(dir)){
