...
if(!dir.exists(here::here("data", "lily", "data", "fingerprint_res_temporal2", paste0(size, "longsubset")))){
  dir.create(here::here("data", "lily", "data", "fingerprint_res_temporal2", paste0(size, "longsubset")), recursive = TRUE)
}
...
for(f in folds$fold){
  ...
  if(length(ids) > 0) {
    ...
    if(size == 8018){
      ...
      dat_nzv = read_rds(here::here("data", "lily", "data", paste0("dat_nzv_train_temporal2", size, "_", f, ".rds"))) %>%
        mutate(id = as.character(id)) %>%
        filter(id %in% ids_all)
      dat_nzv_test = read_rds(here::here("data", "lily", "data", paste0("dat_nzv_test_temporal2", size, "_", f, ".rds"))) %>%
        mutate(id = as.character(id)) %>%
        filter(id %in% ids_all)
    }
    ...
    for(id in ids){
      ...
      i = i + 1
      outfile = here::here("data", "lily", "data", "fingerprint_res_temporal2", paste0(size, "longsubset"), paste0(id, ".rds"))
      dir = dirname(outfile)
      if(!dir.exists(dir)){
