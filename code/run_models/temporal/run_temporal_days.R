...
filenames = readRDS(here::here("data", "lily", "data", "fingerprint_folds_temporal_days.rds"))
...
if(length(ids) > 0){
  ...
  dat_nzv = read_rds(here::here("data", "lily", "data", "dat_nzv_train_temporal_days.rds")) %>%
    mutate(id = as.character(id)) %>%
    filter(id %in% ids_all)
  dat_nzv_test = read_rds(here::here("data", "lily", "data", "dat_nzv_test_temporal_days.rds")) %>%
    mutate(id = as.character(id)) %>%
    filter(id %in% ids_all)
  ...
  for(id in ids){
    ...
    i = i + 1
    outfile = here::here("data", "lily", "data", "fingerprint_res_temporal_days", size, paste0(id, ".rds"))
    dir = dirname(outfile)
    if(!dir.exists(dir)){
